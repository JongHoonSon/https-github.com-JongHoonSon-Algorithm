...
function solution(stones, k) {
  ...
  // 최대로 통과할 수 있는 사람의 수 (최솟값부터 시작해서 갱신)
  let maxPass = left;

  // left보다 right가 같거나 크면 반복
  while (left <= right) {
    mid = Math.floor((left + right) / 2);

    // checkStone이 true를 리턴하면
    if (checkStone(stones, mid, k)) {
      // left를 mid+1로 변경하고, 최소로 통과할 수 있는 사람의 수를 mid로 갱신함
      left = mid + 1;
      maxPass = Math.max(mid, maxPass);

      // checkStone이 false 리턴하면
    } else {
      // right를 mid-1로 변경함
      right = mid - 1;
    }
...
// 정답은 mid 더 낮은 곳에 있다.
// 따라서 이 경우 right를 mid -1로 설정한다.

// 만약모든 stones[i]의 값에서 mid를 뻈는데 0보다 작은 원소가 k번 이상 연속으로 나오지 않는 경우
// 징검다리를 mid명이 건널 수 있다는 뜻이다.
// 따라서 최대로 건널 수 있는 사람의 수를 mid로 갱신하고
// left를 mid+1로 변경하여 더 높은 곳에서 탐색을 진행한다.

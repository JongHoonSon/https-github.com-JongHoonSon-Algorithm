...
function DFS(x, depth) {
  ...
  // DFS for문이 끝날 경우 수행됨
  // 더 이상 탐색할 수 있는 곳이 없으므로,
  // 현재 탐색했던 곳을 false로 하고 함수를 빠져나옴
  visited[x] = false;

  // 현재 탐색했던 곳을 false로 하는 이유 :

  // 1 -> 2 -> 3 순서로 탐색을 진행했고,
  // 3번 노드에서 더이상 탐색이 불가능하며,
  // 2번 노드는 3번 노드과 연결되어 있지만 4번 노드와도 연결되어 있다고 가정하면
  // 다음번에 진행되어야하는 탐색은 2번 노드를 탐색중인 DFS의 for문의 i값이 1 증가하여
  // 4번 노드를 탐색하게 된다.
  // 이 경우 1 -> 2 -> 4 순서로 탐색을 진행한 것이 되므로
  // 1 -> 2 -> 3 에서 탐색했던
  // 기존에 탐색했던 3번 노드의 visited을 false로 변경하여
  // 1 -> 2 -> 4 를 탐색할 때 3번 노드를 탐색할 수 있는 상태로 변경하기 위함이다.

  // + 좋은 점 하나 더
  // 이런 방식으로 하면 DFS가 모든 노드에 대해 실행될 때 (39번째 줄)
  // 일일히 visited의 값을 false로 초기화할 필요가 없다.
  // DFS를 진행하는 과정에서 알아서 visited를 정리하면서 나오므로
}

...
let brokenBtnArray = [];
if(brokenBtnNum >= 2) {
    brokenBtnArray = inputs[2].trim().split(' ').map(v=>+v);
...
const result = Math.min(onlyNumberBtn, onlyPlusMinusBtn, (higherMinNum - targetNum)+higherMinNum.toString().length, (targetNum - lowerMinNum)+lowerMinNum.toString().length);
